...
class IAM(BaseDataModule):
    """
    IAM consists of 9,862 text lines
        The validation and train set are merged
    ...
    def line_regions_by_id(self):
        """Return a dict from name of IAM form to a list of (x1,x2,y1,y2)"""
...
def _get_line_strings_from_xml_file(filename: str) -> List[str]:
    ...
    xml_line_element = xml_root_element.findall("handwritten-part/line")
    return [el.attrib["text"].replace("&quot;", '"') for el in xml_line_element]
...
def _get_line_region_from_xml_element(xml_line) -> Dict[str, int]:
    """
    @params: 
        xml_line: xml element with attribute x,y,width,height 
    """
